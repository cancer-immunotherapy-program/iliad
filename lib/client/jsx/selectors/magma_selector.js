...
export const selectModelDocuments = Reselect.createSelector(
  ...
  (model, record_names)=>{
    ...
    if('documents' in model){
      if(record_names == 'all') record_names = Object.keys(model.documents);
...
export const selectAllRevisions = Reselect.createSelector(
  selectModels,
  (models)=>{
    let revisions = {models: {}};
    for(let model_name in models){
      if(Object.keys(models[model_name].revisions).length > 0){
        revisions.models[model_name] = {
          revisions: models[model_name].revisions
        };
      }
    }
