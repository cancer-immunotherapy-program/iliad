...
export const fetchManifests = (exchange)=>{
  ...
  let uri = Routes.manifests_fetch_path(TIMUR_CONFIG.project_name);
...
export const createManifest = (manifest, exchange)=>{
  ...
  let uri = Routes.manifests_create_path(TIMUR_CONFIG.project_name);
  return exchange.fetch(uri, route_opts)
    .then(checkStatus)
    .then(parseJSON);
};
...
export const getConsignments = (manifests, exchange)=>{
  ...
  let uri = Routes.consignment_path(TIMUR_CONFIG.project_name);
  return exchange.fetch(uri, route_opts)
    .then(checkStatus)
    .then(parseJSON);
};
