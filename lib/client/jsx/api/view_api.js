...
export const getView = (model_name, exchange)=>{
  ...
  let uri = Routes.retrieve_view_path(APP_CONFIG.project_name, model_name);
...
export const updateView = (view_obj, exchange)=>{
  ...
  let uri = Routes.update_view_path(APP_CONFIG.project_name);
  return exchange.fetch(uri, route_opts)
    .then(checkStatus)
    .then(parseJSON);
};
...
export const deleteView = (view_obj, exchange)=>{
  ...
  let uri = Routes.delete_view_path(APP_CONFIG.project_name);
  return exchange.fetch(uri, route_opts)
    .then(checkStatus)
    .then(parseJSON);
};
