...
export const getView = (model_name, tab_name, exchange)=>{
  ...
  let exchangePromise = exchange.fetch(Routes.view_path(APP_CONFIG.project_name, model_name), route_opts)
    .then(checkStatus)
    .then(parseJSON);
...
export const updateView = (view_obj, exchange)=>{
  ...
  let exchangePromise = exchange.fetch(Routes.update_view_json_path(APP_CONFIG.project_name), route_opts)
    .then(checkStatus)
    .then(parseJSON);
...
export const deleteView = (view_obj, exchange)=>{
  ...
  let exchangePromise = exchange.fetch(Routes.delete_view_json_path(APP_CONFIG.project_name), route_opts)
    .then(checkStatus)
    .then(parseJSON);
