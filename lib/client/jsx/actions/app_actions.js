...
export const requestView = (model_nm, rec_nm, tab_nm, success, error)=>{
  return (dispatch)=>{
    ...
    let localSuccess = (response)=>{
      ...
      // Make the call for regular data.
      dispatch(
        requestDocuments({
          model_name: model_nm,
          exchange_name: `tab ${tab_nm} for ${model_nm} ${rec_nm}`,
          record_names: [rec_nm],
          attribute_names: getAttributes(tab, false)
        })
      );

      /*
       * Make the calls for the table data. The delay is essential. If Magma
       * gets too many calls at once it will drop connections.
       */
      let delay = 0; // In milliseconds.
      getAttributes(tab, true).forEach((attr)=>{
        delay += 100;
        setTimeout(()=>{
          dispatch(
            requestDocuments({
              model_name: model_nm,
              exchange_name: `tab ${tab_nm} for ${model_nm} ${rec_nm} ${attr}`,
              record_names: [rec_nm],
