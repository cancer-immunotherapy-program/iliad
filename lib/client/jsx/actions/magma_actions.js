...
export const addDocumentsForTemplate = (model_name, documents, project_name)=>{
  return {
    type: 'ADD_DOCUMENTS',
    project_name,
...
export const sendRevisions = (args, prj_nm = TIMUR_CONFIG.project_name)=>{
  return (dispatch)=>{
    ...
    let exchng = new Exchange(dispatch, `revisions-${model_name}`);
    let form_data = setFormData(revisions, prj_nm, model_name);
    postRevisions(form_data, exchng, prj_nm)
      .then(localSuccess)
      .catch(localError);
  }
};
