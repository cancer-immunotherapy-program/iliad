...
export default class ManifestScript extends React.Component{
  scroll(){
    ...
    if(this.editor && this.script){
      if(is_editing){
        this.script.scrollTop = this.editor.scrollTop;
      }
      else{
        this.editor.scrollTop = this.script.scrollTop;
      }
    }
  }
  ...
  render(){
    ...
    let className = 'manifest-script-container'
    if(is_editing) className += ' editing';

    let code_mirror_props = {
      options: {
